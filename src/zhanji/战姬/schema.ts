...
const getRules = () => ({
  ...
  COMBAT_TYPE: {
    '均衡型': [0.20, 0.20, 0.20, 0.20, 0.20],
    '强攻型': [0.30, 0.20, 0.10, 0.10, 0.30],
    '重盾型': [0.10, 0.40, 0.10, 0.30, 0.10],
    '魔导型': [0.10, 0.10, 0.40, 0.30, 0.10],
    '敏捷型': [0.20, 0.10, 0.20, 0.10, 0.40]
  },
  NATURE: {
    '固执': [0, 2], '孤僻': [0, 1], '勇敢': [0, 4], '调皮': [0, 3],
    '内敛': [2, 0], '稳重': [2, 1], '冷静': [2, 4], '马虎': [2, 3],
    '胆小': [4, 0], '急躁': [4, 1], '开朗': [4, 2], '天真': [4, 3],
    '大胆': [1, 0], '悠闲': [1, 4], '淘气': [1, 2],
    '温和': [3, 0], '自大': [3, 4], '慎重': [3, 2],
    '坦率': [null, null], '认真': [null, null], '害羞': [null, null]
  },
  FORM_BONUS: {
    '无': { atk: 0, def: 0, spA: 0, spD: 0, spd: 0 },
    '天使化': { atk: 0, def: 0, spA: 0.25, spD: 0.25, spd: 0.25 },
    '恶魔化': { atk: 0, def: 0, spA: 0.25, spD: 0.25, spd: 0.25 },
    '精灵化': { atk: 0, def: 0, spA: 0.20, spD: 0.20, spd: 0.20 },
    '兽人化': { atk: 0.20, def: 0.20, spA: 0, spD: 0, spd: 0.20 }
  },
...
const CollectionRoomSchema = z.object({
  已解锁: z.boolean().prefault(false),
...
const TrainerSchema = z.object({
  ...
  特殊装备: z.array(TrainerEquipmentSchema)
...
const TaskSchema = z.object({
  ...
  优先级: safeEnum(['低', '中', '高', '紧急'], '中'),
  图标: z.string().prefault(''),
  发布者: z.string().prefault('系统'),
  背景色: z.string().prefault('')
});
...
const SexualStateSchema = z.object({
  胸部: z.object({
    尺寸: z.string().prefault(''),
    乳头状态: z.string().prefault('粉嫩'),
    开发度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
    敏感度: safeEnum(['迟钝', '普通', '敏感', '过敏', '淫乱'], '普通'),
    当前状态: z.string().prefault('正常'),
    特殊标记: z.string().prefault('')
  }).prefault({}),
  阴部: z.object({
    处女膜: safeEnum(['完整', '已破', '无'], '完整'),
    紧致度: safeEnum(['紧致', '普通', '松弛', '松垮'], '紧致'),
    开发度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
    敏感度: safeEnum(['迟钝', '普通', '敏感', '过敏', '淫乱'], '普通'),
    当前状态: z.string().prefault('正常'),
    特殊标记: z.string().prefault('')
  ...
  后庭: z.object({
    开发度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
    紧致度: safeEnum(['紧闭', '紧致', '普通', '松弛'], '紧闭'),
    敏感度: safeEnum(['迟钝', '普通', '敏感', '过敏', '淫乱'], '迟钝'),
    当前状态: z.string().prefault('正常'),
    特殊标记: z.string().prefault('')
  }).prefault({}),
  子宫: z.object({
    状态: safeEnum(['未开发', '轻度开发', '中度开发', '完全开发', '子宫脱垂'], '未开发'),
    敏感度: safeEnum(['迟钝', '普通', '敏感', '过敏', '淫乱'], '迟钝'),
    当前状态: z.string().prefault('正常'),
    特殊标记: z.string().prefault('')
  }).prefault({}),
  口腔: z.object({
    技巧: safeEnum(['生疏', '普通', '熟练', '精通', '大师'], '生疏'),
    开发度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
    敏感度: safeEnum(['迟钝', '普通', '敏感', '过敏'], '普通'),
    深喉适应: safeEnum(['无法接受', '勉强', '适应', '精通'], '无法接受'),
    当前状态: z.string().prefault('正常'),
    特殊标记: z.string().prefault('')
  }).prefault({}),
  整体淫乱度: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
  性经验次数: z.coerce.number().prefault(0),
  高潮体质: safeEnum(['难以高潮', '普通', '容易高潮', '连续高潮体质', '潮吹体质'], '普通'),
  当前性欲: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0)
}).prefault({});
...
const SingleCharacterSchema = z.object({
  ...
  性格: safeEnum([
    '固执', '孤僻', '勇敢', '调皮', '内敛', '稳重', '冷静', '马虎',
    '胆小', '急躁', '开朗', '天真', '大胆', '悠闲', '淘气',
    '温和', '自大', '慎重', '坦率', '认真', '害羞'
  ], '认真'),
  战斗类型: safeEnum(['均衡型', '强攻型', '重盾型', '魔导型', '敏捷型'], '均衡型'),
  元素属性: safeEnum(['地', '火', '水', '风', '光', '暗'], '地'),
  好感度: z.coerce.number().transform(v => _.clamp(v, -100, 200)).prefault(0),
  堕落值: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(0),
...
const EnemyTrainerProfileSchema = z.object({
  名字: z.string().prefault(''),
  类型: safeEnum(['骄傲型', '复仇型', '理智型', '胆小型', '宠爱独占型', '工具使用型', '虐待调教型', '共享轮用型', '展示炫耀型', '洗脑奴化型', '放置忽视型', '未知'], '未知'),
  意志状态: safeEnum(['坚定', '动摇', '崩溃', '未知'], '未知'),
  持有捕捉球: z.boolean().prefault(false),
  捕捉球类型: safeEnum(['普通球', '高级球', '超级球', '魅惑之球'], '普通球'),
  捕捉辅助器: z.boolean().prefault(false),
}).prefault({});

const BattleConsoleSchema = z.object({
  进行中: z.boolean().prefault(false),
  己方出战: z.string().prefault(''),
  敌方出战: z.string().prefault(''),
  己方队伍: z.array(z.string()).prefault([]),
  敌方队伍: z.array(z.string()).prefault([]),
  战斗类型: safeEnum(['普通', '捕获', 'BOSS'], '普通'),
  敌方训练家信息: EnemyTrainerProfileSchema,
}).prefault({});

const AUTO_EVENTS = [
  { id: '特殊事件_喜欢', condition: (char) => char.好感度 >= 100 && !char.已触发事件['特殊事件_喜欢'] },
  { id: '特殊事件_初次亲吻', condition: (char) => char.好感度 >= 105 && !char.已触发事件['特殊事件_初次亲吻'] },
  { id: '特殊事件_深情告白', condition: (char) => char.好感度 >= 110 && char.已触发事件['特殊事件_喜欢'] && !char.已触发事件['特殊事件_深情告白'] },
  { id: '特殊事件_形态觉醒', condition: (char) => char.好感度 >= 150 && !char.已触发事件['特殊事件_形态觉醒'] },
  { id: '特殊事件_羁绊技能', condition: (char) => char.好感度 >= 200 && !char.已触发事件['特殊事件_羁绊技能'] }
];
...
export const Schema = z.object({
  ...
  })).transform(items => _.pickBy(items, i => i.数量 > 0)).prefault({}),
  任务列表: z.record(z.string(), TaskSchema)
    ...
    .prefault({}),
  _任务奖励映射: z.record(z.string(), z.any()).prefault({}),
  角色数据: z.record(z.string(), SingleCharacterSchema).prefault({}),
  战斗控制台: BattleConsoleSchema,
})
.transform((data) => {
  const RULES = getRules();
  ...
  if (data.训练家) {
    ...
    if (trainer.据点.已解锁 && trainer.据点.收藏室.已解锁) {
      ...
      if (collectionRoom.等级 > 0) {
        ...
        // 计算升级费用
        const upgradeCosts = { 0: 0, 1: 1000, 2: 3000, 3: 8000, 4: 20000, 5: 0 };
        collectionRoom.升级费用 = upgradeCosts[collectionRoom.等级] || 0;
      }
    }
    ...
    if (trainer.据点.已解锁 && trainer.据点.工作区.已解锁) {
      ...
      if (workArea.等级 > 0) {
        ...
        // 计算升级费用
        const upgradeCosts = { 0: 0, 1: 1000, 2: 3000, 3: 8000, 4: 20000, 5: 0 };
        workArea.升级费用 = upgradeCosts[workArea.等级] || 0;
      }
    }
  }
  ...
  if (currentDate !== lastSettlementDate) {
      ...
      if (collectionRoom.等级 > 0 && collectionRoom.当前使用.length > 0) {
        ...
        for (const senkiName of collectionRoom.当前使用) {
          if (data.角色数据[senkiName]) {
            ...
            const oldAffection = char.好感度;
            char.好感度 = _.clamp(oldAffection + affectionBonus, -100, 200);
            console.log(`  [收藏室] ${senkiName} 好感度 +${affectionBonus} (${oldAffection} → ${char.好感度})`);
            ...
            console.log(`  [收藏室] ${senkiName} HP恢复 +${hpRecover}, MP恢复 +${mpRecover}`);
          }
        }
      }
    }
    ...
    if (data.训练家.据点.已解锁 && data.训练家.据点.工作区.已解锁) {
      ...
      if (workArea.等级 > 0 && workArea.当前使用.length > 0) {
        ...
        const totalIncome = incomePerPerson * workArea.当前使用.length;

        data.金币 += totalIncome;
        console.log(`  [工作区] 产出金币 +${totalIncome} (${incomePerPerson}/人 × ${workArea.当前使用.length}人)`);
      }
    }

    // ====== 更新结算日期 ======
    data.世界._上次结算日期 = currentDate;
    console.log(`[日常结算] 完成，更新结算日期为 ${currentDate}`);
  }
  ...
  for (const name in data.角色数据) {
    ...
    if (char.从属训练家 === data.玩家名 || char.从属训练家 === '') {
      if (char.归属状态 === '已储存') {
        储存列表.push(name);
      }
    }
  ...
  for (const name in data.角色数据) {
    ...
    if (char.等级 > 1 && safeExp === 0) {
      const minExp = calculateMinExpForLevel(char.等级);
      char.经验值.当前值 = minExp;
    } else {
      char.等级 = newLevel;
    }
    ...
    char.特防 = stats.特防;
    char.速度 = stats.速度;
    char.生命值.最大值 = stats.HP最大值;
    char.法力值.最大值 = stats.MP最大值;

    if (isLevelUp || char.生命值.当前值 <= 0) char.生命值.当前值 = stats.HP最大值;
    if (isLevelUp || char.法力值.当前值 <= 0) char.法力值.当前值 = stats.MP最大值;

    char.生命值.当前值 = _.clamp(char.生命值.当前值, 0, stats.HP最大值);
    char.法力值.当前值 = _.clamp(char.法力值.当前值, 0, stats.MP最大值);

    // 整体淫乱度计算
    if (char.性器状态) {
      const s = char.性器状态;
      const weights = { 胸部: 0.15, 阴部: 0.30, 阴蒂: 0.15, 后庭: 0.20, 子宫: 0.10, 口腔: 0.10 };
      ...
      const wombMap = { '未开发': 0, '轻度开发': 25, '中度开发': 50, '完全开发': 75, '子宫脱垂': 100 };
      totalDev += (wombMap[s.子宫?.状态] || 0) * weights.子宫;
